		...
		order by pnum
			...
			$cnt=0;
			 foreach($result as $row){
				...
				$view_problemset[$cnt][1]="问题&nbsp;&nbsp;".$PID[$cnt];
			...
			$i=0;
			 foreach($result as $row){
				...
				$view_contest[$i][1]= "<a href='contest.php?cid=".$row['contest_id']."'>".$row['title']."</a>";
                                $info_contest[$i][3]=$row['title'];
				$info_contest[$i][0]=$row['start_time'];
                                $start_time=strtotime($row['start_time']);
				$info_contest[$i][1]=$row['end_time'];
                                $end_time=strtotime($row['end_time']);
				$now=time();
                                
                                
        $length=$end_time-$start_time;
        $info_contest[$i][2]=formatTimeLength($length);
