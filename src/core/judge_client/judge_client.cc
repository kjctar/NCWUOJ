...
#ifdef _mysql_h
void update_testpoint(struct testpoint *head)
{
	struct testpoint* p=head->next;
	struct testpoint* free;
	char tbname[20]="testpoint";
	...
	while(p!=NULL)
	{
		...
		sprintf(sql, "SELECT * FROM testpoint WHERE solution_id=%d and filename='%s'",
				p->solution_id,p->filename);
		mysql_real_query(conn, sql, strlen(sql));
		res = mysql_store_result(conn);
		...
		if ((row = mysql_fetch_row(res))) {
				...
				sprintf(sql,
					"UPDATE %s SET result=%d,time=%d,memory=%d WHERE solution_id=%d and filename='%s'",
						tbname,	 p->result,  p->time,   p->memory >> 10,  p->solution_id, p->filename);
					if (mysql_real_query(conn, sql, strlen(sql))) {
						//              printf("..update failed! %s\n",mysql_error(conn));
					}
			}
				 ...
				 p->memory=p->memory>>10;
					sprintf(sql,
					"insert into %s(solution_id,filename, result, time ,memory ) values(%d,'%s',%d,%d,%d)",
						tbname,	p->solution_id, p->filename ,p->result, p->time,  p->memory);
					if (mysql_real_query(conn, sql, strlen(sql))) {
						//              printf("..update failed! %s\n",mysql_error(conn));
					}
...
int main(int argc, char** argv) {
    ...
    struct testpoint *head= new testpoint;  /*************************************链表头*********************************/
...
/*******************************************************************获取样例的时间  内存   输出文件名*****************************/
					struct testpoint *p=new testpoint;
					p->solution_id=solution_id;
				    if (ACflg == OJ_TL) {
						p->time = time_lmt * 1000;
					}
					else{
						p->time=usedtime;
					}
