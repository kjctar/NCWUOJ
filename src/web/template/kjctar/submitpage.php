...
layui.use('layer', function(){ 
  ...
  $(document).on('click', '#myselftest', function() {
  //触发事件
...
<font style='color:white;line-height:30px'>
<?php if (isset($id)){?>
...
<div id="test_board" style="display:none">
<center>
<br><br>
<?php echo $MSG_Input?>:<br><textarea style="width:90%" cols=40 rows=5 id="input_text" name="input_text" ><?php echo $view_sample_input?></textarea>

<hr><?php echo $MSG_Output?>:<br>
<textarea style="width:90%" cols=10 rows=5 id="out" name="out" >SHOULD BE:
...
<input id="Submit" class="layui-btn layui-btn-xs" type=button value="<?php echo $MSG_SUBMIT?>" onclick="do_submit();" >
<?php if (isset($OJ_TEST_RUN)&&$OJ_TEST_RUN){
echo '<input id="myselftest" class="layui-btn layui-btn-normal layui-btn-xs" type=button value="自测" >';
}
 ?>
...
<?php if($OJ_ACE_EDITOR){ ?>
	<pre style="width:100%;height:93vh;margin:0px 0px 0px 0px;border-color:#303030"  id="source"><?php echo htmlentities($view_src,ENT_QUOTES,"UTF-8")?> </pre>
...
<tr><td>测试点<td> 时间<td>内存 <td>结果  </tr>
</table>-->
	...
	problem_id.value='<?php if (isset($cid))echo $cid?>';
            ...
            var hint= Array("未登陆",
		          	"Verification Code Wrong!",
      					"Where do find this link? No such problem",
      					"You Can't Submit Now Because Your are not invited by the contest   or the contest is not running!!",
      					"You are not invited!",
      					"No Such Problem!",
      					"Code too short!",
      					"Code too long!",
      					"You should not submit more than twice in 10 seconds.....");
            var ret=Array("Pending",
                          "Pending_Rejudging",
                          "Compiling",
                          "Running_Judging",
                          "<font style='color:green'>Accepted</font>",
                          "Presentation Error",
                          "<font style='color:red'>Wrong Answer</font>",
                          "Time Limit Exceeded",
                          ...
                          "Compilation Error",
                          "Compile_OK");
          var ajax_url = "submit.php"; //表单目标 
　　      var ajax_type = $("#frmSolution").attr('method'); //提交方法 
　　      var ajax_data = $("#frmSolution").serialize(); //表单数据 
...
　　      $.ajax({ 
    ...
    　　       success:function(msg,status){
            ...
            //             alert(msg);
                         ...
                         {
                         	msg=hint[msg];
                         	 spop({template:'<p class="spop-title">'+msg+'</p>',
                                                               position:'top-center',
                                                                autoclose: 2000});
                         }
                         else{
                              spop({template:'<table id="result"   class="layui-table"  lay-size="sm" ><tr><td>测试点<td> 时间<td>内存 <td>结果 </tr></table>',
                                     icon:false });
                               
                               var changed=-1;
                                var stay=self.setInterval(function(){
                                  $.ajax({
                                    type:'post',
                                    url:'template/kjctar/finish.php',
                                    data:{'sid':msg},
                                                ...
                                                if(result!=changed)
                                                   {
                                                     changed=result;
                                                     if(changed<4)
                                                      spop({template:'<p  class="spop-title">'+ret[changed]+'  &nbsp;<i  class="fa fa-spinner fa-pulse"></i></p>',
                                                               
                                                                group: 'satus'});
                                                     
                                                     else if(changed==4)
                                                       spop({template:'<p class="spop-title">'+ret[changed]+'</p>',
                                                                style:'success',
                                                                group: 'satus'
                                                       ...
                                                       spop({template:'<p class="spop-title">'+ret[changed]+'</p>',
                                                          ...
                                                          $.ajax({
                                                          type:'post',
                                                          url:'template/kjctar/api_ceinfo.php',
                                                          data:{'sid':msg},
                                            
                                                          success:function(result,status)
                                                          {
                                                    spop({template:'<h4 class="spop-title">'+ret[changed]+'</h4><hr><p class="spop-title">'+result+'</p>',
                                                          style:'warning',
                                                          group: 'satus'});
                                                 ...
                                                 if(result>=4)
                                                 {
                                                   ...
                                                   window.clearInterval(stay);
                                                      $.ajax({
                                                          type:'post',
                                                          url:'template/kjctar/testpoint.php',
                                                          data:{'sid':msg},
                                                          dataType: 'json',
                                                          success:function(result,status)
                                                          {
                                                                     if(status == 'success')
                                                                     {
                                                                       ...
                                                                       // result=JSON.parse(result);
                                                                        ...
                                                                        for(i=0;i<count;i++)
                                                                        {
                                                                         ...
                                                                         //alert(result[i].filename+"--"+result[i].time+result[i].memory+result[i].result);
 
                            document.getElementById("result").innerHTML+="<tr><td>"+result[i].filename+"<td>"+result[i].time+"<td>"+result[i].memory+"<td>"+ret[result[i].result]+"</tr>";
                                                                        }
                                                                     }
                                                          }
                                                       }); 
