...
if($OJ_TEMPLATE!="classic") 
	$judge_color=Array("btn gray btn-xs","btn btn-info btn-xs","btn btn-warning btn-xs","btn btn-warning btn-xs","btn btn-success btn-xs","btn btn-danger btn-xs","btn btn-danger btn-xs","btn btn-warning btn-xs","btn btn-warning btn-xs","btn btn-warning btn-xs","btn btn-warning btn-xs","btn btn-warning btn-xs","btn btn-warning btn-xs","btn btn-info btn-xs");
...
for ($i=0;$i<$rows_cnt;$i++){
        ...
        if ($row['contest_id']>0) {
                ...
                if (isset($_SESSION[$OJ_NAME.'_'.'administrator']))
                	...
                	$view_status[$i][1]= $row['user_id'] /*"<a href='contestrank.php?cid=".$row['contest_id']."&user_id=".$row['user_id']."#".$row['user_id']."'>".$row['user_id']."</a>"i*/;
        }else{
                if (isset($_SESSION[$OJ_NAME.'_'.'administrator']))
	...
	$view_status[$i][3]="<span class='hidden' style='display:none' result='".$row['result']."' ></span>";
        ...
        if ($flag){
                ...
                if (!(isset($_SESSION[$OJ_NAME.'_'.'user_id'])&&strtolower($row['user_id'])==strtolower($_SESSION[$OJ_NAME.'_'.'user_id']) || isset($_SESSION[$OJ_NAME.'_'.'source_browser']))){
                        ...
                        $view_status[$i][6]= "<a href=javascript:showcode('showsource.php?id=".$row['solution_id']."');>".$language_name[$row['language']]."</a>";
			if($row["problem_id"]>0){
                        	if ($row['contest_id']>0) {
                                	$view_status[$i][6].= "/<a  href=javascript:showcode('submitpage.php?cid=".$row['contest_id']."&pid=".$row['num']."&sid=".$row['solution_id']."');>Edit</a>";
                        	}else{
                                	$view_status[$i][6].= "/<a  href=javascript:showcode('submitpage.php?id=".$row['problem_id']."&sid=".$row['solution_id']."');>Edit</a>";
                        	}
			}
                }
